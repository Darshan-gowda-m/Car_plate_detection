...
class Result(db.Model):
    ...
    detection_time = db.Column(db.Float)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'detection_confidence': float(self.detection_confidence) if self.detection_confidence else 0.0,
            'detection_time': float(self.detection_time) if self.detection_time else 0.0,
            ...
            'best_ocr_confidence': float(self.best_ocr_confidence) if self.best_ocr_confidence else 0.0,
            'ocr_time': float(self.ocr_time) if self.ocr_time else 0.0,
...
with app.app_context():
    db.create_all()  # This ensures models are registered




app.Result = Result
app.BatchJob = BatchJob
app.db = db

print(f"✅ Models attached to app: app.Result = {app.Result}, app.db = {app.db}")

# ==================== NOW IMPORT AND REGISTER BLUEPRINTS ====================
# Import blueprints AFTER models are attached to app
from backend.api.results import results_api
from backend.api.system import system_api
from backend.api.upload import upload_api

# Now register blueprints - they will use app.db.session.query(Result) instead of Result.query
...
def get_results_stats():
    """Get statistics - DIRECT IMPLEMENTATION"""
    try:
        with app.app_context():
            from sqlalchemy import func
            from datetime import datetime, timedelta
            
            # Use the local Result model
            ...
            if total_results == 0:
                return jsonify({
                    ...
                    'engine_stats': {
                        'easyocr': {'count': 0, 'avg_confidence': 0, 'success_rate': 0},
                        'tesseract': {'count': 0, 'avg_confidence': 0, 'success_rate': 0},
                        'google': {'count': 0, 'avg_confidence': 0, 'success_rate': 0}
                    },
                    'daily_stats': [
...
def export_results():
    ...
    with app.app_context():
        query = db.session.query(Result).order_by(desc(Result.timestamp))
        results = query.all()
        ...
        if format_type == 'json':
            ...
            # Write header
            writer.writerow([
                'ID', 'Filename', 'Timestamp', 'Detection Method',
                'Detection Confidence', 'Best OCR Engine', 'Extracted Text',
                'OCR Confidence', 'Processing Time (s)', 'Status'
            ])
            ...
            for result in results:
                writer.writerow([
                    ...
                    result.detection_confidence or 0,
                    result.best_ocr_engine or '',
                    result.best_ocr_text or '',
                    result.best_ocr_confidence or 0,
                    result.processing_time or 0,
                    result.status or ''
                ])
            
            response = make_response(output.getvalue())
            response.headers['Content-Disposition'] = 'attachment; filename=plate_results.csv'
            response.headers['Content-type'] = 'text/csv'
            
            return response
            
        else:
            return jsonify({'error': 'Unsupported format', 'success': False}), 400
...
@app.route('/api/results/stats', methods=['GET'])
def get_results_stats():
    """Get statistics - redirect to the correct endpoint"""
    try:
        # Use proper app context
        with app.app_context():
            # Get the same data as results_api.get_statistics()
            from sqlalchemy import func
            from datetime import datetime, timedelta
            
            # Get overall statistics
            total_results = Result.query.count()
            successful_results = Result.query.filter(Result.status == 'completed').count()
            ...
            max_confidence = db.session.query(
                func.max(Result.best_ocr_confidence)
            ).scalar() or 0
            
            min_confidence = db.session.query(
                func.min(Result.best_ocr_confidence)
            ).scalar() or 0
            ...
            for engine in engines:
                engine_query = Result.query.filter(
                    Result.best_ocr_engine == engine
                )
                
                engine_count = engine_query.count()
                if engine_count > 0:
                    ...
                    engine_stats[engine] = {
                        'count': engine_count,
                        'avg_confidence': float(engine_avg_conf),
                        'success_rate': (engine_count / total_results * 100) if total_results > 0 else 0
                    }
...
def system_config():
    ...
    try:
        ...
        config = {
            ...
            'ocr': {
                'engines_available': list(app.ocr_manager.engines.keys()) if app.ocr_manager else [],
    ...
    try:
        with app.app_context():
            ...
            if result.enhanced_path:
                files_to_delete.append(Path(result.enhanced_path))
            if result.visualization_path:
                files_to_delete.append(Path(result.visualization_path))
            if result.annotated_video_path:
                files_to_delete.append(Path(result.annotated_video_path))
...
def get_system_stats():
    ...
    try:
        with app.app_context():
            # Database statistics
            total_results = Result.query.count()
            completed_results = Result.query.filter(Result.status == 'completed').count()
            failed_results = Result.query.filter(Result.status == 'failed').count()
            
            # Confidence statistics
            avg_confidence = db.session.query(func.avg(Result.best_ocr_confidence)).scalar() or 0
            max_confidence = db.session.query(func.max(Result.best_ocr_confidence)).scalar() or 0
            ...
            for engine in engines:
                count = Result.query.filter(Result.best_ocr_engine.contains(engine)).count()
                if count > 0:
                    engine_avg_conf = db.session.query(
                        func.avg(Result.best_ocr_confidence)
                    ).filter(
            ...
            vehicle_types = db.session.query(
                Result.vehicle_type,
                func.count(Result.vehicle_type).label('count')
            ).group_by(Result.vehicle_type).all()
            ...
            system_metrics = {
                ...
                'transformer_loaded': transformer_detector is not None,
                'ocr_engines_loaded': len(ocr_manager.engines) if ocr_manager else 0,
                'deep_ocr_loaded': deep_ocr is not None,
...
def process_single_image():
    ...
    viz_path = None
    overall_quality = 0  # Initialize the variable
    ...
    try:
        ...
        if use_yolo and detector:
            try:
                ...
                for i, det in enumerate(yolo_detections):
                    ...
                    print(f"     • YOLO Det {i+1}: bbox={bbox}, size={width}x{height}, aspect={aspect:.2f}, conf={det['confidence']:.3f}")
        ...
        if use_transformer and transformer_detector:
            try:
                ...
                # Debug: Show all transformer detections
                for i, det in enumerate(transformer_detections):
                    bbox = det['bbox']
                    width = bbox[2] - bbox[0]
                    height = bbox[3] - bbox[1]
                    aspect = width / height if height > 0 else 0
                    print(f"     • Transformer Det {i+1}: bbox={bbox}, size={width}x{height}, aspect={aspect:.2f}, conf={det['confidence']:.3f}")
        ...
        # Debug: Show final detections
        if filtered_detections:
            print(f"   • Final plate details:")
            for i, det in enumerate(filtered_detections):
                bbox = det['bbox']
                width = bbox[2] - bbox[0]
                height = bbox[3] - bbox[1]
                aspect = width / height if height > 0 else 0
                print(f"     • Plate {i+1}: bbox={bbox}, size={width}x{height}, aspect={aspect:.2f}, "
                      f"conf={det['confidence']:.3f}, detector={det.get('detector', 'unknown')}")
        ...
        if filtered_detections:
            ...
            try:
                ...
                print(f"   • Visualization generated: {viz_time:.3f}s")
        ...
        try:
            with app.app_context():
                ...
                result_entry = Result(
                    ...
                    ocr_time=float(sum(r['processing_time']['ocr'] for r in all_results) if all_results else 0),
        ...
        # Calculate overall quality score using the function
        ...
        response = {
            ...
            'processing_summary': {
                ...
                'detection_time': float(detection_time),
                'ocr_time': float(sum(r['processing_time']['ocr'] for r in all_results) if all_results else 0),
                'database_time': float(time.time() - db_start),
            ...
            'files': file_urls,
            'timestamp': datetime.utcnow().isoformat(),
            'quality_score': float(overall_quality)  # Now this variable is defined
        }
...
def get_results():
    """Get processing results with pagination - FIXED VERSION"""
    try:
        with app.app_context():
            # Get query parameters
            ...
            # Build query using db.session.query()
            query = db.session.query(Result)  # <-- CHANGED HERE
            ...
            # Paginate - For db.session.query(), we need to use offset/limit
            total = query.count()
...
@app.route('/api/system/health-metrics', methods=['GET'])
def get_health_metrics():
    """Simple health metrics endpoint"""
    try:
        import psutil
